...
describe("GET: Get all recipes", () => {
  it("Should get the first 20 recipes", (done) => {
    request(baseUrl)
      .get("/api/recipes/all")
      .end((err, res) => {
    ...
    request(baseUrl)
      ...
      .end((err, res) => {
        ...
        expect(
          data.some((item) => {
            return (
              item.title.includes("steak") ||
              item.summary.includes("steak") ||
              item.instructions.includes("steak") ||
              Object.values(JSON.parse(item.ingredients)).some((ingredient) =>
                ingredient.includes("steak")
              ) ||
              Object.values(JSON.parse(item.types)).some((type) =>
                type.includes("steak")
              )
            );
          })
